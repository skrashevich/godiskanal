...
Переменные окружения:
  ...
  OPENAI_BASE_URL  Базовый URL API (если не указан --api-url)`,
	...
	"flag.version":        "Показать версию и выйти",
	...
	"known.header":        "ИЗВЕСТНЫЕ МЕСТА (%d)",

	// ── Root / multi-user ────────────────────────────────────────────────────
	"root.home_hint":    "  \033[36mℹ Запуск от root — проверяем известные места для %s (не /var/root)\033[0m",
	"root.multi_user":   "  \033[36mℹ Запуск от root — известные места для %d учётных записей\033[0m",
	"root.scan_default": "  \033[2mПуть сканирования по умолчанию: %s (переопределите через --path)\033[0m",
	...
	"llm.model":     "  Модель: \033[1m%s\033[0m",
	"llm.provider":  "  API-провайдер: \033[1m%s\033[0m  \033[2m(%s)\033[0m",
	"llm.analyzing": "  \033[2mАнализирую...\033[0m",
	...
	// ── LLM prompts ─────────────────────────────────────────────────────────
	"llm.prompt.header":       "## Анализ диска macOS (godiskanal)\n\n",
	"llm.prompt.scan":         "**Путь сканирования:** %s\n",
	"llm.prompt.root":         "**Режим:** запуск от root — учитываются системные и мультипользовательские пути\n",
	"llm.prompt.disk":         "**Диск:** %s всего, %s использовано (%.0f%%), %s свободно\n\n",
	"llm.prompt.top_dirs":     "### Топ директорий по размеру:\n",
	"llm.prompt.node_modules": "\n### Крупные node_modules (≥200 МБ):\n",
	"llm.prompt.node_item":    "- %s — %s\n",
	"llm.prompt.tm":           "\n**Time Machine:** %d локальных снимков на диске (часто по несколько ГБ). Удаляйте только если откат не нужен: `tmutil deletelocalsnapshots /`\n",
	"llm.prompt.known":        "\n### Известные macOS / dev локации (из сканера):\n",
	"llm.prompt.cleanable":    " [очистка в godiskanal -i]",
	"llm.prompt.manual":       " [только команда из подсказки — не rm -rf]",
	"llm.prompt.size_unknown": "размер вне дерева сканирования",
	"llm.prompt.path":         "  Путь: %s\n",
	"llm.prompt.desc":         "  Описание: %s\n",
	"llm.prompt.suggested":    "  Рекомендуется: %s\n",
	"llm.prompt.request": "\n\n## Задача\n" +
		"Дай **5–8** приоритетных рекомендаций по освобождению места. Используй ТОЛЬКО пути и размеры из данных выше — не выдумывай папки.\n\n" +
		"Для каждой рекомендации — такой блок:\n" +
		"### N. Краткий заголовок (~ожидаемый выигрыш)\n" +
		"- **Зачем:** одно предложение — что это и почему много весит\n" +
		"- **Путь:** точный путь из данных\n" +
		"- **Риск:** низкий | средний | высокий — что сломается при ошибке\n" +
		"- **Действие:** точная команда в терминале или шаги в приложении (предпочитай «Рекомендуется:» из данных, а не rm -rf)\n\n" +
		"Правила:\n" +
		"- Сортируй по эффекту; пропускай пункты < ~50 МБ, если диск не забит под завязку\n" +
		"- Для [очистка в godiskanal -i] укажи godiskanal -i, когда это безопаснее\n" +
		"- Для [только команда из подсказки] — только команда из подсказки, без удаления через Finder и без rm -rf на всю папку\n" +
		"- Не предлагай удалять /System, /usr, ~/Library/Mail и исходники активных проектов\n" +
		"- Если указаны снимки Time Machine — включи их при count > 0\n\n" +
		"В конце блок **Дальше:** одна строка про godiskanal -i и godiskanal -b --llm.",
	...
	"llm.system.describe.auto": "Ты эксперт по очистке диска macOS в режиме браузера godiskanal.\n" +
		"Пользователь листает папки; ты видишь путь, размеры и подсказки сканера.\n" +
		"Ответь за 2-3 коротких предложения: можно ли удалять, лучшая команда очистки, крупнейший вложенный элемент.\n" +
		"Используй «Рекомендуется» из сканера; не предлагай rm -rf на целые папки Library.\n" +
		"Отвечай на русском языке.",
	"llm.system.describe.analyze": "Ты эксперт по очистке диска macOS. Пользователь запросил глубокий анализ (клавиша i) в браузере godiskanal.\n" +
		"Только пути и размеры из запроса. Предпочитай штатную очистку приложений, а не массовое удаление.\n" +
		"Формат: краткое резюме, затем нумерованные шаги с Риск (низкий/средний/высокий) и Действие (команда или UI).\n" +
		"Если в подсказке только ручная команда — дай только её, без удаления через Finder.\n" +
		"Отвечай на русском языке.",
	"llm.system.analyze": "Ты эксперт по очистке диска macOS. Пользователь запустил godiskanal --llm после реального сканирования.\n" +
		"Каждая рекомендация должна опираться только на переданные данные скана.\n" +
		"Предпочитай штатные и приложенческие команды очистки, а не удаление целых папок Library.\n" +
		"Будь осторожен с оценкой риска; предупреждай перед разрушительными действиями.\n" +
		"Используй понятный markdown. Отвечай на русском языке.",
	...
	"browser.confirm.app_related": "  Включая связанные данные приложения (кэши, настройки, контейнеры, …):\n",
	"browser.confirm.related":     "  — связано с %s",
  ...
  D               Удалить текущий элемент
      Удаление .app из Applications предлагает связанные файлы в Library
`,
...
`,
	...
	"browser.llm.auto_ask": "\nКраткий совет (2-3 предложения). Используй подсказки сканера, если есть.\n" +
		"Укажи: удалять или нет, лучшую команду очистки и крупнейший вложенный элемент для просмотра.",
	"browser.llm.analyze":    "Глубокий анализ: %s\nТип: %s | Общий размер: %s\n",
	"browser.llm.contents":   "\nСодержимое (топ 15 по размеру):\n",
	"browser.llm.more_entries": "  ... (ещё +%d)\n",
	"browser.llm.known_match":     "\nСканер знает эту локацию: **%s**\n",
	"browser.llm.known_desc":      "  Описание: %s\n",
	"browser.llm.known_suggested": "  Рекомендуется: %s\n",
	"browser.llm.known_manual":    "  Важно: только команда из подсказки — не массовое удаление в Finder.\n",
	"browser.llm.known_cleanable": "  Также можно очистить через godiskanal -i после полного скана.\n",
	"browser.llm.questions": "\nСтруктурированный ответ:\n" +
		"1. **Что это** (одно предложение)\n" +
		"2. **Безопасная очистка** — оценка места и точная команда или шаги (предпочитай «Рекомендуется» выше)\n" +
		"3. **Риски** — низкий / средний / высокий и что сломается\n" +
		"4. **Дальше** — крупнейшая вложенная папка для просмотра, если есть",
	...
	"loc.System Caches.desc":        "Системные кэши приложений",
	"loc.System Caches.note":        "Удаляйте только знакомые подпапки кэша; приложения пересоздадут их",
	"loc.System Logs.desc":          "Системные логи и логи служб",
	"loc.System Logs.note":          "Старые логи обычно безопасно удалять; macOS создаст новые при необходимости",
	"loc.Temporary files.desc":      "Временные файлы в /private/tmp",
	"loc.Temporary files.note":      "Безопасно очищать, если не идёт установка или работа приложений",
	"loc.User temp caches.desc":     "Временные файлы и кэши пользователей (varfolders)",
	"loc.User temp caches.note":     "Смешанные данные разных пользователей — проверьте крупные подпапки перед удалением",
	"loc.Installer leftovers.desc":  "Остатки установщиков обновлений macOS",
	"loc.Installer leftovers.note":  "Могут занимать много места; удаляйте только после успешной установки обновлений",
}
